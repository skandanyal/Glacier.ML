#ifndef SIMPLELINEARREGRESSION_HPP
#define SIMPLELINEARREGRESSION_HPP
...
namespace Glacier {
	...
	public:
		...
		void predict_print(const std::vector<float>& x_predict, const std::vector<float>& y_predict);
...
// constructor
inline Glacier::Simple_Linear_Regression::Simple_Linear_Regression(std::vector<float> &x, std::vector<float> &y) {
	if(y.size() != x.size()) {
 ...
 * |_ calculate_c()
 */
inline void Glacier::Simple_Linear_Regression::train() {
...
inline float Glacier::Simple_Linear_Regression::calculate_m() {
...
inline float Glacier::Simple_Linear_Regression::calculate_c() {
	return (sum_y - m * sum_x) / n;
}

inline void Glacier::Simple_Linear_Regression::print_values() const {
	LOG_INFO("Regression coefficients: ");
	std::cout << "m: " << m << "\nc: " << c << "\n\n";
}

// predict() for float x, returns float result
inline float Glacier::Simple_Linear_Regression::predict(float a) {
	return m * a + c;
}
 ...
 */
inline void Glacier::Simple_Linear_Regression::analyze(std::vector<float> &x_test, std::vector<float> &y_test) {
	if (x_test.size() <= 2) {
	...
	x_test_size = (float) x_test.size(), x_test_mean = Glacier::Utils::mean(x_test);
...
inline void Glacier::Simple_Linear_Regression::hypothesis(std::string param, float t_statistic, int dof) {
	LOG_INFO("Hypothesis testing: " + param);
	double p_value = Glacier::Utils::get_p_value(t_statistic, dof);
...
inline void Glacier::Simple_Linear_Regression::print_confidence_intervals(const std::string& param, float measure, float SE) {
...
inline void Glacier::Simple_Linear_Regression::predict_print(const std::vector<float>& x_pred, const std::vector<float>& y_pred) {
...
inline std::vector<float> Glacier::Simple_Linear_Regression::predict(const std::vector<float> &x_pred) {
