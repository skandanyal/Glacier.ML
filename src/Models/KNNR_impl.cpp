...
#pragma omp parallel for default(none) shared(mean, std_dev, nrows, ncols, X)
    ...
    inline float KNNRegressor::predict(std::vector<float> &x_pred) {
...
shared(answer)
    ...
    inline std::vector<float> KNNRegressor::predict(std::vector<std::vector<float>> &x_pred) {
    ...
    inline void KNNRegressor::print_predict(std::vector<std::vector<float>> &x_pred, std::vector<float> &y_pred) {
...
#pragma omp parallel for reduction(+:ss_res) reduction(+:ss_tot)
    ...
    inline void KNNRegressor::analyze(std::vector<std::vector<float>> &x_test, std::vector<float> &y_test) {
